...
function createEntityCard(entity) {
    ...
    return `
        <div class="entity-card ${confidenceClass}-confidence" data-id="${entity.id}" data-type="${entity.entity_type}">
            <div class="entity-header">
                <div class="entity-title">
                    <h3 class="entity-name">${entity.entity_name}</h3>
                    <span class="entity-type-badge ${entity.entity_type}">${entity.entity_type}</span>
                </div>
            ...
            <div class="entity-body">
                ...
                ${data.source_context ? `
                    <div class="entity-field">
                        ...
                        <div class="source-context">"${data.source_context}"</div>
                    </div>
                ` : ''}
                
                <div class="target-info">
                    <strong>Will be added to:</strong> ${entity.target_file.split('/').pop()}
                </div>
            </div>
    ...
    if (data.description) {
        fields.push(`
            <div class="entity-field">
                ...
                <div class="entity-field-value">${data.description}</div>
            </div>
        `);
    }
        ...
        if (data.relationship) {
            fields.push(`
                <div class="entity-field">
                    ...
                    <div class="entity-field-value">${data.relationship}</div>
                </div>
            `);
        }
    } else if (type === 'faction') {
        if (data.goals) fields.push(`<div class="entity-field"><label>Goals:</label><div class="entity-field-value">${data.goals}</div></div>`);
        if (data.leadership) fields.push(`<div class="entity-field"><label>Leadership:</label><div class="entity-field-value">${data.leadership}</div></div>`);
        if (data.territory) fields.push(`<div class="entity-field"><label>Territory:</label><div class="entity-field-value">${data.territory}</div></div>`);
        if (data.relationship) fields.push(`<div class="entity-field"><label>Relationship:</label><div class="entity-field-value">${data.relationship}</div></div>`);
    } else if (type === 'location') {
        if (data.significance) fields.push(`<div class="entity-field"><label>Significance:</label><div class="entity-field-value">${data.significance}</div></div>`);
    } else if (type === 'item') {
        if (data.properties) fields.push(`<div class="entity-field"><label>Properties:</label><div class="entity-field-value">${data.properties}</div></div>`);
    } else if (type === 'alias') {
        if (data.real_identity) fields.push(`<div class="entity-field"><label>Real Identity:</label><div class="entity-field-value">${data.real_identity}</div></div>`);
        if (data.purpose) fields.push(`<div class="entity-field"><label>Purpose:</label><div class="entity-field-value">${data.purpose}</div></div>`);
        if (data.appearance) fields.push(`<div class="entity-field"><label>Appearance:</label><div class="entity-field-value">${data.appearance}</div></div>`);
    }
    ...
    if (entity.entity_type === 'npc') {
        extraFields.innerHTML = `
            <div class="form-group">
                ...
                <input type="text" id="edit-relationship" value="${data.relationship || ''}">
            </div>
        `;
        ...
        extraFields.innerHTML = `
            <div class="form-group">
                <label>Goals:</label>
                <input type="text" id="edit-goals" value="${data.goals || ''}">
            </div>
            <div class="form-group">
                <label>Leadership:</label>
                <input type="text" id="edit-leadership" value="${data.leadership || ''}">
            </div>
        `;
    }
